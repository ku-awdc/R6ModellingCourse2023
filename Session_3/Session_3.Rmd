...
Classes can contain many other classes


## Composition: Example

Simulation contains population, spread, surveillance


## Composition in R6

Either pass in other classes or generate them in initialize

[Unless you want a shared instance e.g. logger]


## Mixing Composition and Inheritance

Simulation will take a Population, but who knows what kind
...
Is a compiled language (C++ thing)

Can be used instead of (or in addition to) composition/inheritance

More performant as the optimiser sees all
...
Based on a mixture of R6 and C++

Uses inheritance, composition, and TMP

More next time:  Install RTools (and LaTeX)
...
1. Create a virtual base class "Farm" that:

2. Create a Pig Farm class that inherits from farm and:

3. Create a simulation class that takes a Farm (or derived class) input and runs a simulation

4. Create a Dairy Farm class that inherits from farm and:

5. Convince yourself that the Pig and Dairy farm classes are interchangeable


## END


Create a simulation class that takes a farm as input, and does the looping over
